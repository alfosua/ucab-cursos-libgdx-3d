...
const initialData: CourseItem[] = [
  { id: 1, module: '1. Introducción a LibGDX 3D', topic: 'Conceptos Básicos', description: 'Conceptos básicos, configuración del entorno y conceptos de 3D.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 2, module: '1. Introducción a LibGDX 3D', topic: 'Estructura del Proyecto', description: 'Módulos principales y clase Main.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 3, module: '1. Introducción a LibGDX 3D', topic: 'Clase Base 3D', description: 'Configuración de la clase base 3D, cámara y entorno.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 4, module: '2. Renderizado Básico', topic: 'Modelo 3D Simple', description: 'Renderizado de un modelo 3D básico usando ModelBuilder.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 5, module: '2. Renderizado Básico', topic: 'Escena GLTF Estática', description: 'Carga y renderizado de una escena GLTF estática.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 6, module: '2. Renderizado Básico', topic: 'Animación 3D', description: 'Reproducción de animaciones desde un archivo GLTF.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 7, module: '2. Renderizado Básico', topic: 'Cámara FPS', description: 'Configuración de una cámara en primera persona (FPS).', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 8, module: '3. Generación Procedural', topic: 'Primitivas Geométricas', description: 'Creación de formas geométricas básicas por código.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 9, module: '3. Generación Procedural', topic: 'Mallas Personalizadas', description: 'Generación de mallas 3D personalizadas vértice a vértice.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 10, module: '3. Generación Procedural', topic: 'Terrenos con Heightmaps', description: 'Generación de terreno procedural usando mapas de altura.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 11, module: '4. Iluminación y Entorno', topic: 'Tipos de Luces', description: 'Tipos de luces: Direccional, Puntual y Foco.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 12, module: '4. Iluminación y Entorno', topic: 'Sombras Dinámicas', description: 'Implementación de sombras direccionales.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 13, module: '4. Iluminación y Entorno', topic: 'Skybox Procedural', description: 'Creación de un Skybox procedural con gradientes.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 14, module: '4. Iluminación y Entorno', topic: 'Reflejos PBR (IBL)', description: 'Reflejos basados en imágenes (Image-Based Lighting).', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 15, module: '5. Shaders Avanzados', topic: 'Renderizado Fotorrealista', description: 'Renderizado fotorrealista con materiales PBR.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 16, module: '5. Shaders Avanzados', topic: 'Shaders Personalizados', description: 'Creación e integración de un shader GLSL personalizado.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 17, module: '5. Shaders Avanzados', topic: 'Estilo Cell Shading', description: 'Modificación de shaders PBR para lograr un estilo Cell Shading.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 18, module: '6. Físicas y Optimización', topic: 'Físicas con Bullet', description: 'Integración de físicas 3D usando la librería Bullet.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 19, module: '6. Físicas y Optimización', topic: 'Físicas Avanzadas', description: 'Mallas de colisión, restricciones y fuerzas.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 20, module: '6. Físicas y Optimización', topic: 'Raycasting con Bullet', description: 'Uso de Raycasting para detectar colisiones con el ratón.', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
  { id: 21, module: '6. Físicas y Optimización', topic: 'Entornos Masivos', description: 'Exploración de entornos grandes (Conceptos y teoría).', status: 'not-started', link: 'https://github.com/alfosua/3dexp' },
];

export default function App() {
  const [data, setData] = useState<CourseItem[]>(() => {
    const saved = localStorage.getItem('libgdx-3d-course-v1-progress');
    if (saved) {
      const parsedSaved: CourseItem[] = JSON.parse(saved);
      // Merge saved status with the new titles and descriptions from initialData
      return initialData.map(initialItem => {
        const savedItem = parsedSaved.find(s => s.id === initialItem.id);
        return savedItem ? { ...initialItem, status: savedItem.status } : initialItem;
      });
    }
    return initialData;
  });
  ...
  const [lastSeen, setLastSeen] = useState<{topicId: number, slideIndex: number} | null>(() => {
    const saved = localStorage.getItem('libgdx-3d-course-v1-last-seen');
    return saved ? JSON.parse(saved) : null;
  });

  useEffect(() => {
    localStorage.setItem('libgdx-3d-course-v1-progress', JSON.stringify(data));
  }, [data]);

  useEffect(() => {
    if (lastSeen) {
      localStorage.setItem('libgdx-3d-course-v1-last-seen', JSON.stringify(lastSeen));
    } else {
      localStorage.removeItem('libgdx-3d-course-v1-last-seen');
    }
  }, [lastSeen]);
  ...
  const confirmReset = () => {
    setData(initialData);
    setLastSeen(null);
    localStorage.removeItem('libgdx-3d-course-v1-progress');
    localStorage.removeItem('libgdx-3d-course-v1-last-seen');
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex flex-col justify-center">
              <h1 className="font-semibold text-sm sm:text-lg tracking-tight leading-tight">Curso de Desarrollo 3D con LibGDX</h1>
      ...
      <main className="flex-1 w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <a 
              href="https://github.com/alfosua/3dexp"
              target="_blank"
      ...
      {showCreditsModal && (
        <div className="fixed inset-0 z-50 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-8 animate-in fade-in zoom-in-95 duration-200 text-center">
            ...
            <p className="text-slate-600 mb-8">Has finalizado con éxito el Curso de Desarrollo 3D con LibGDX.</p>
